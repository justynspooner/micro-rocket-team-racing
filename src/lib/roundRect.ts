...
(() => {
  ...
  function roundRect(
    ...
    function toDOMPointInit(value: { x: any; y: any; z: any; w: any }) {
      const { x, y, z, w } = value;
      return { x, y, z, w };
    }

    function parseRadiiArgument(value: { [x: string]: any } | null) {
    ...
    function toCornerPoint(value: { x: any; y: any }) {
    ...
    function fixOverlappingCorners(
      corners: { x: number; y: number }[] | [any, any, any, any]
    ) {
  ...
  function getConstructorName(instance: { constructor: { name: any } }) {
